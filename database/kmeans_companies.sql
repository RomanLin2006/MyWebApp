DELIMITER // 
CREATE PROCEDURE kmeans_companies_clustering(IN k INT) 
BEGIN 
    ...
    -- Проверяем, достаточно ли компаний с координатами
    SELECT COUNT(*) INTO companies_count 
    FROM companies 
    WHERE latitude IS NOT NULL AND longitude IS NOT NULL;
    
    IF companies_count < k THEN
        SELECT CONCAT('Ошибка: доступно только ', companies_count, ' компаний с координатами, а нужно минимум ', k) as error;
        LEAVE;
    END IF;
    ...
    -- Инициализация центроидов случайными компаниями
    SET @cluster_num = 0; 
    ...
    main_loop: WHILE NOT converged DO 
        ...
        -- Назначение каждой компании ближайшему центроиду
        ...
        -- Пересчет центроидов как среднее координат компаний в кластере
        ...
        -- Обработка пустых кластеров (переназначение случайными компаниями)
        UPDATE temp_centroids cent 
        LEFT JOIN ( 
    ...
    UPDATE companies SET cluster_id = NULL; 
 
    -- Применение новых кластеров
    UPDATE companies c 
    JOIN temp_assignments a ON c.id = a.company_id 
    SET c.cluster_id = a.cluster_id; 
     
    -- Возврат статистики по кластерам
    SELECT  
        c.cluster_id, 
        COUNT(*) as companies_count, 
        ...
        CONCAT('Итераций выполнено: ', iteration) as info 
    FROM companies c 
